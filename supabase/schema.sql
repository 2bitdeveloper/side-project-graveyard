...
insert into public.site_stats (key, value) values ('page_views', 0) on conflict do nothing;

create or replace function public.increment_page_views()
returns bigint language plpgsql security definer set search_path = public, pg_temp as $$
declare v bigint;
begin
  update public.site_stats set value = value + 1 where key = 'page_views' returning value into v;
  return v;
end $$;
